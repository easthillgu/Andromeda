...
local function UpdateColor(self, event, runeID, alt)
	...
	if rune then
		...
		if color then
			rune:GetStatusBarTexture():SetVertexColor(color[1], color[2], color[3])
		end
	else
		...
		for i = 1, #element do
			...
			color = (specColor and self.colors.runes[spec]) or self.colors.runes[bar.runeType] or self.colors.power.RUNES
