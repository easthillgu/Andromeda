...
local function UpdateColor(self, event, unit)
	...
	local r, g, b, color
	if(element.colorDisconnected and not (unit and UnitIsConnected(unit))) then
	...
	elseif(element.colorSmooth) then
		if oUF.isRetail then
			...
			r, g, b = oUF:ColorGradient(maxValue == 0 and 0 or (curValue / maxValue), unpack(element.smoothGradient or self.colors.smooth))
		end
