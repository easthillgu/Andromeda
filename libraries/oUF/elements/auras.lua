...
local function CreateButton(element, index)
	...
	local button = CreateFrame('Button', name and name .. 'Button' .. index or nil, element, 'BackdropTemplate')
	...
	if(show) then
		if button.Overlay then
			if (isDebuff and element.showDebuffType) or (not isDebuff and element.showBuffType) or element.showType then
				...
				local color = colors[dispelName] or colors.none or { 0, 0, 0 }
				if color[1] then
					button.Overlay:SetVertexColor(color[1] or 0, color[2] or 0, color[3] or 0)
				else
					button.Overlay:SetVertexColor(color[1] or 0, color[2] or 0, color[3] or 0)
				end
				button.Overlay:Show()
