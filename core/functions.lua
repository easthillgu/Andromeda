        ...
        function F.IsBoA(bagID, slotID)
            ...
            if link then
                for key in pairs(boaStr) do
                    if strfind(link, key) then return true end
                end
            end
            return false
        end
...
do
    ...
    do
        function F:CreateFS(font, size, flag, text, colour, shadow, anchor, x, y)
            ...
            if font then
                fs:SetFont(font, size, type(flag) == 'string' and flag or (flag and 'OUTLINE' or ''))
            else
    ...
    do
        ...
        function F:CreateBD(alpha)
            ...
            -- 3.80.1 compat: inject backdrop methods if missing
            if not self.SetBackdrop then
                self.SetBackdrop = function(frame, backdrop) end
                self.SetBackdropColor = function(frame, r, g, b, a) end
                self.SetBackdropBorderColor = function(frame, r, g, b, a) end
                self.GetBackdropColor = function(frame) return 0, 0, 0, 1 end
                self.GetBackdropBorderColor = function(frame) return 0, 0, 0, 1 end
            end
        ...
        function F.CreateBDFrame(parent, a, gradient)
            ...
            parent.__bg:SetFrameLevel(lvl == 0 and 0 or lvl - 1)

            F.CreateBD(parent.__bg, a)

            if gradient then
                parent.__gradient = F.CreateGradient(parent.__bg)
            end
...
do
    ...
    do
        ...
        function F:StripTextures(kill)
            if not self then return end  -- 3.80.1: nil guard for missing frames
    ...
    do
        ...
        function F:ReskinScroll()
            if self == nil then return end  -- 3.80.1
    ...
    do
        ...
        function F.ReskinArrow(button, direction)
            if not button then return end
            F.StripTextures(button)
            button:SetSize(16, 16)
            -- button:SetDisabledTexture(C.Assets.Textures.Backdrop)
