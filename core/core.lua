...
loader:SetScript('OnEvent', function(self, _, addon)
    ...
    -- Guard: configs.lua may not have loaded yet
    ...
    if not _G.ANDROMEDA_ADB['ProfileIndex'] then
        _G.ANDROMEDA_ADB['ProfileIndex'] = {}
    end
    ...
    -- Guard: TexturesList only available after gui/gui.lua loads
    if GUI.TexturesList then
        if _G.ANDROMEDA_ADB['NameplateCustomTexture'] ~= '' then
            ...
            if not GUI.TexturesList[_G.ANDROMEDA_ADB['NameplateTextureIndex']] then
                _G.ANDROMEDA_ADB['NameplateTextureIndex'] = 1
            end
