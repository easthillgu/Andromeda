...
local function UpdateActionCamera()
    ...
    if Misc and Misc.UpdateActionCamera then
...
local function UpdateScreenSaver()
    -- 3.80.1: ScreenSaver module may not be loaded yet, guard
    local ScreenSaver = F:GetModule('ScreenSaver')
    if ScreenSaver then
...
local function MuteAnnoyingSounds()
    local Misc = F:GetModule('Misc')
    if Misc and Misc.MuteAnnoyingSounds then
...
local function UpdateCombatPulse()
    if INFOBAR and INFOBAR.UpdateCombatPulse then
