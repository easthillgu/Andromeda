...
function GUI:SetupActionBarSize(parent)
    ...
    for _, v in ipairs(datas.barPet) do
        ...
        createSlider(scroll, mKey, v.key, v.text, v.min, v.max, v.step, v.value, 20, offset - 450, function() ACTIONBAR:UpdatePetBar() end)
        offset = offset - 65
    end
    ...
    for _, v in ipairs(datas.barStance) do
        ...
        createSlider(scroll, mKey, v.key, v.text, v.min, v.max, v.step, v.value, 20, offset - 500, function() ACTIONBAR:UpdateStanceBar() end)
        offset = offset - 65
    end
    ...
    for _, v in ipairs(datas.barExtra) do
        ...
        createSlider(scroll, mKey, v.key, v.text, v.min, v.max, v.step, v.value, 20, offset - 550, function() ACTIONBAR:UpdateStanceBar() end)
        offset = offset - 65
    end
    ...
    for _, v in ipairs(datas.barVehicle) do
        ...
        createSlider(scroll, mKey, v.key, v.text, v.min, v.max, v.step, v.value, 20, offset - 600, function() ACTIONBAR:UpdateStanceBar() end)
        offset = offset - 65
    end
end
...
local function UpdateVignettingVisibility()
    -- 3.80.1: Vignetting module has no implementation, guard
    if VIGNETTING and VIGNETTING.UpdateVisibility then
