...
do
    ...
    function UNITFRAME.PostCreateButton(element, button)
        ...
        button.timer = F.CreateFS(button, font, fontSize, 'OUTLINEMONOCHROME')
    ...
    -- Andromeda expected (element, button, unit, data). Remap params.
    function UNITFRAME.PostUpdateButton(element, unitStr, auraButton, index, position, auraDuration, auraExpiration, auraDispelName, auraStealable)
        -- Swap: elvui's unitStr→Andromeda's button param was actually button
        -- elvui's auraButton→Andromeda's unit param was actually unit name
        local button = auraButton
        local unit = unitStr
        local data = {}
        local duration = auraDuration
        local expiration = auraExpiration
        local debuffType = auraDispelName
        local isStealable = auraStealable
    ...
    local function UpdatePlayerAuraPosition(self)
        local specIndex = GetSpecialization and GetSpecialization() or GetActiveSpecGroup and GetActiveSpecGroup()
    ...
    function NAMEPLATE.NameplateAuraFilter(frame, element, unit, button, aura, name, icon,
        ...
        local isMyAura = isCasterPlayer[sourceUnit]
...
do
    ...
    function UNITFRAME:CreateDebuffsIndicator(self)
        ...
        for i = 1, 3 do
            ...
            local outline = _G.ANDROMEDA_ADB.FontOutline
            button.count = F.CreateFS(parentFrame, C.Assets.Fonts.Small, 11, 'OUTLINEMONOCHROME')
            button.count:ClearAllPoints()
