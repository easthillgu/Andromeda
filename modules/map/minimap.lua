...
function MAP:RemoveBlizzStuff()
    ...
    -- 3.80.1: hide all named children safely
    ...
    -- Hide BlizzArt (3.80.1 safe, only hides if exists)
    local frames = {
        'MinimapBorderTop',
        'MinimapNorthTag',
        'MinimapBorder',
...
function MAP:CreateQueueStatusButton()
    if not _G.QueueStatusButton then return end
    local Minimap = _G.Minimap
...
function MAP:BuildDropDown()
    ...
    if _G.MiniMapTrackingDropDown_Initialize then
        _G.UIDropDownMenu_Initialize(dropdown, _G.MiniMapTrackingDropDown_Initialize, 'MENU')
    end

    -- 3.80.1: MinimapCluster.Tracking doesn't exist, use global MiniMapTrackingButton
    local trackingBtn = _G.MiniMapTrackingButton
    if trackingBtn then
        hooksecurefunc(trackingBtn, 'Update', function()
