...
function MAP:AddCoords()
    ...
    local titleContainer = WorldMapFrame.BorderFrame.TitleContainer
    if not titleContainer then return end

    local outline = _G.ANDROMEDA_ADB.FontOutline
    playerCoords = F.CreateFS(titleContainer, C.Assets.Fonts.Bold, 12, outline or nil, '', nil, outline and 'NONE' or 'THICK')
    playerCoords:SetPoint('TOPLEFT', 10, -6)
    cursorCoords = F.CreateFS(titleContainer, C.Assets.Fonts.Bold, 12, outline or nil, '', nil, outline and 'NONE' or 'THICK')
...
function MAP:SetupWorldMap()
    ...
    -- 3.80.1: UIPanelLayout not available
    pcall(WorldMapFrame.SetAttribute, WorldMapFrame, 'UIPanelLayout-area', nil)
    pcall(WorldMapFrame.SetAttribute, WorldMapFrame, 'UIPanelLayout-enabled', false)
    pcall(WorldMapFrame.SetAttribute, WorldMapFrame, 'UIPanelLayout-allowOtherPanels', true)
