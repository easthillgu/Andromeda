...
TOOLTIP.MountIDs = {}
-- 3.80.1: C_MountJournal may not exist; nil guard at module load
...
function TOOLTIP:OnTooltipSetUnit()
    ...
    if C.DB.Tooltip.HideInCombat and InCombatLockdown() and not (C_PetBattles and C_PetBattles.IsInBattle and C_PetBattles.IsInBattle()) then
    ...
    -- TOOLTIP.AddMythicPlusScore(self, unit) -- 3.80.1: Retail M+ not available
    TOOLTIP.ScanTargets(self, unit)
    -- TOOLTIP.AddCovenantInfo() -- 3.80.1: covenant.lua removed (Retail SL)
...
function TOOLTIP:SetUnitAura(unit, index, filter)
    ...
    if id then
        local mountText
        if TOOLTIP.MountIDs[id] and C_MountJournal then
...
function TOOLTIP:OnLogin()
    ...
    TOOLTIP:MountSource()
    TOOLTIP:HyperLink()
    -- TOOLTIP:CovenantInfo() -- 3.80.1: covenant.lua removed
    TOOLTIP:Achievement()
    -- 3.80.1: azerite.lua / paragon.lua removed (Retail BFA/DF)
