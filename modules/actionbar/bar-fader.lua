...
function ACTIONBAR:FadeParent_OnEvent(event)
    -- Persist showGrid state across events
    if event == 'ACTIONBAR_SHOWGRID' then
        self._showGridLock = true
    elseif event == 'ACTIONBAR_HIDEGRID' then
        self._showGridLock = nil
    end
...
function ACTIONBAR:BarFader()
    ...
    -- showGrid is set on bars before BarFader runs, init from config
    ACTIONBAR.fadeParent._showGridLock = C.DB.Actionbar.Grid
