    ...
    if IsPlayerSpell(spellID) then
        ...
        if self.db.enableCastLine then
            ...
            if frame.icon then
                ...
                frame:Show()
                if frame.ag then frame.ag:Play() end
            end
    ...
    if db.enableCastLine then
        ...
        if pool.EnumerateInactive then
            for _, f in pool:EnumerateInactive() do
                if pool.resetterFunc then pool:resetterFunc(f) end
            end
        end
    end
...
function keyFeedback:CreateFeedbackButton(autoKeyup)
    ...
    if autoKeyup then
        mirror:SetScript('OnUpdate', function(self, elapsed)
            ...
            if timePassed >= 0.05 and self.pushed then
                mirror:SetButtonState('NORMAL')
                self.pushed = false
            end

            if timePassed >= 0.1 then
                local alpha = 0.3 - timePassed
                if alpha <= 0 then
                    alpha = 0
                    self:Hide()
                end
