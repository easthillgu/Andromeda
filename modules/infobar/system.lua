...
local function ColorGradient(perc, ...)
    if perc >= 1 then return select(select('#', ...) - 2, ...) end
    if perc <= 0 then return ... end
    local n = select('#', ...) / 3
    local seg, rel = math.modf(perc * (n - 1))
    local r1, g1, b1 = select((seg * 3) + 1, ...)
    local r2, g2, b2 = select((seg * 3) + 4, ...)
...
local function BuildAddonList()
    ...
    for i = 1, numAddons do
        local _, title, _, loadable = (C_AddOns and C_AddOns.GetAddOnInfo or GetAddOnInfo)(i)
        if loadable then
