...
local function safeRegisterEvent(event, handler)
    local ok = pcall(F.RegisterEvent, F, event, handler)
    if not ok then
        -- 3.80.1: event may not exist
    end
end
...
local function checkInstanceType()
    ...
    if (instID and isIgnoredZone[instID]) or (instanceType == 'scenario' and (maxPlayers == 3 or maxPlayers == 6)) then
        safeRegisterEvent('VIGNETTE_MINIMAP_UPDATED', nil) -- unregister via pcall below
        pcall(F.UnregisterEvent, F, 'VIGNETTE_MINIMAP_UPDATED', onEvent)
    else
        safeRegisterEvent('VIGNETTE_MINIMAP_UPDATED', onEvent)
    end
